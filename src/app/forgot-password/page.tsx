            ...
            {isSuccess ? (
              <div className="mt-8 space-y-4">
                {devResetLink ? (
                  <div className="rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-3 text-xs text-amber-900">
                    ...
                    <Link
                      href={devResetLink}
                      className="mt-2 block break-all font-semibold text-brand-600 underline"
                    >
                      {devResetLink}
                    </Link>
                  </div>
                ...
                <Link
                  href="/login"
                  className="inline-flex w-full items-center justify-center rounded-xl bg-gradient-to-r from-brand-500 to-brand-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-brand-500/25 transition hover:brightness-110"
                >
                  {t("backToLogin")}
                </Link>
              </div>
              ...
              <form onSubmit={handleSubmit} className="mt-8 space-y-5">
                <label className="block">
                  <span className="mb-1.5 block text-sm font-medium text-slate-700">{t("emailLabel")}</span>
                  <input
                    type="email"
                    required
                    autoComplete="email"
